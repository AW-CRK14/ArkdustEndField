 ...
 * 天气计划器将根据当前的气候参数进行运算，并生成一个{@link Weather.ActivatedStateInServer}。同时，对于一条线路上生成的天气是否转化为天灾变种也由WeatherScheduler决定。<br>
 * 在给出的气候参数中，如果尝试获取了一个没有被放入的{@link ClimateParameter.ActivatedState}，将会返回其对应气候参数的默认值。<br>
 * 如果您想将一个气候参数设置为非默认值但是不变更，请使用{@link ClimateParameter.ActivatedState#ActivatedState(ClimateParameter,int value)}
 * */
public class WeatherScheduler implements INBTSerializable<CompoundTag> {
    ...
    public CompoundTag serializeNBT() {
        ...
        tag.putString("provider", Registry$Weather.WEATHER_PROVIDER.getKey(provider).toString());
        return tag;
    }
    ...
    public void deserializeNBT(CompoundTag nbt) {
        ...
        this.provider = Registry$Weather.WEATHER_PROVIDER.get(new ResourceLocation(nbt.getString("provider")));
    }
